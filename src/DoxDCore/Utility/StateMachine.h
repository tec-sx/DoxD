...
#define MAX_HSMEVENT_DATA	5
	...
	public:
		...
		SStateEvent() : m_eventType(eSE_None)
#ifdef STATE_DEBUG
			, m_debugContextAt(-1)
#endif
		{
...
#define CALL_STATE_DELETE_FN(subStateIndex)  (*(m_factories[trueStateID].m_deletePtr))
	...
	public:
		static void StateInit(HOST& host, CStateMachineRegistration<HOST>& statemachineReg, STATE*& pState)
		{
			...
			RecursiveToCommonReverse(host, pState->m_currentState, 0, pState, eSE_Init);
			StateMachineHandleEventForState(host, statemachineReg, pState, STATE_DEBUG_RAW_EVENT_LOG(pState, eSE_Enter), 0);
		}
		...
		static void StateRelease(HOST& host, CStateMachineRegistration<HOST>& statemachineReg, STATE*& pState)
		{
			...
			RecursiveToCommon(host, pState->m_currentState, 0, pState, STATE_DEBUG_RAW_EVENT_LOG(pState, eSE_Exit));
			RecursiveToCommon(host, pState->m_currentState, 0, pState, eSE_Release);
		}
			...
			do
			{
				...
				default:
					if (pState->m_currentState != stateResult)
					{
						...
						if (event.GetEventId() >= eSE_Custom)
						{
	...
	private:
		...
		void TransitionFromCurrentToSubState(HOST& host, CStateMachineRegistration<HOST>& stateMachineReg, TStateIndex toSubState)
		{
			...
			CStateHelper<HOST, CStateHierarchy<HOST> >::RecursiveToCommon(host, m_currentState, commonParent, pState, eSE_Exit);
			CStateHelper<HOST, CStateHierarchy<HOST> >::RecursiveToCommon(host, m_currentState, commonParent, pState, eSE_Release);

			m_currentState = toSubState;

			CStateHelper<HOST, CStateHierarchy<HOST> >::RecursiveToCommonReverse(host, m_currentState, commonParent, pState, eSE_Init);
			CStateHelper<HOST, CStateHierarchy<HOST> >::StateMachineHandleEventForState(host, stateMachineReg, pState, eSE_Enter, commonParent);
		}
	...
	public:
		...
		void StateMachineUpdateTime(HOST& host, CStateMachineRegistration<HOST>& stateMachineReg, float frameTime, bool debug)
		{
			...
			if (debug)
			{
				...
				SStateEvent debugEvent(eSE_Debug);
