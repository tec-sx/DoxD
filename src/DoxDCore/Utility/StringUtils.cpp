...
namespace DoxD
{
	...
	const char* GetTimeString(int secs, bool useShortForm/*=false*/, bool includeSeconds/*=true*/, bool useSingleLetters/*=false*/)
	{
		...
		ExpandTimeSeconds(secs, d, h, m, s);
		...
		else
		{
			...
			{
				if (d == 1)
					result.Format("%d %s", d, LocalizationUtility::LocalizeString("@ui_day"));
				else if (d > 1)
					result.Format("%d %s", d, LocalizationUtility::LocalizeString("@ui_days"));

				if (h == 1)
					result.Format("%s%s%d %s", result.c_str(), result.empty() ? "" : " ", h, LocalizationUtility::LocalizeString("@ui_hr"));
				else if (h > 1 || d > 0)
					result.Format("%s%s%d %s", result.c_str(), result.empty() ? "" : " ", h, LocalizationUtility::LocalizeString("@ui_hrs"));

				if (m == 1)
					result.Format("%s%s%d %s", result.c_str(), result.empty() ? "" : " ", m, LocalizationUtility::LocalizeString("@ui_min"));
				else if (m > 1 || h > 0 || d > 0)
					result.Format("%s%s%d %s", result.c_str(), result.empty() ? "" : " ", m, LocalizationUtility::LocalizeString("@ui_mins"));

				if (includeSeconds)
				{
					if (s == 1)
						result.Format("%s%s%d %s", result.c_str(), result.empty() ? "" : " ", s, LocalizationUtility::LocalizeString("@ui_sec"));
					else
						result.Format("%s%s%d %s", result.c_str(), result.empty() ? "" : " ", s, LocalizationUtility::LocalizeString("@ui_secs"));
				}
			}
			else
			{
				if (d > 0)
					result.Format("%d %s", d, LocalizationUtility::LocalizeString("@ui_mp_days"));

				if (h > 0 || d > 0)
					result.Format("%s%s%d %s", result.c_str(), result.empty() ? "" : " ", h, LocalizationUtility::LocalizeString("@ui_mp_hrs"));

				if (m > 0 || h > 0 || d > 0)
					result.Format("%s%s%d %s", result.c_str(), result.empty() ? "" : " ", m, LocalizationUtility::LocalizeString("@ui_mp_mins"));

				if (includeSeconds)
				{
					if (s > 0 || (h == 0 && d == 0 && m == 0))
						result.Format("%s%s%d %s", result.c_str(), result.empty() ? "" : " ", s, LocalizationUtility::LocalizeString("@ui_mp_sec"));
				}
			}
		}
	...
	void ExpandTimeSeconds(int secs, int& days, int& hours, int& minutes, int& seconds)
	{
		days = secs / 86400;
		secs -= days * 86400;
		hours = secs / 3600;
