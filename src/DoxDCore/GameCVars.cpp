...
{
	g_pGameCVars->cm_fov = clamp_tpl<float>(g_pGameCVars->cm_fov, 25.f, 80.f);
}
...
void CGamePlugin::RegisterConsoleVars()
{
	assert(m_pConsole);
...
void CGamePlugin::RegisterConsoleCommands()
{
	assert(m_pConsole);
}

void CGamePlugin::UnregisterConsoleCommands()
{
	assert(m_pConsole);
}
