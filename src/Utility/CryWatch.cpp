...
namespace DoxD
{
	...
	static float GetWatchTextYPos()
	{
		...
		float belowConsolePosY = g_pCVars->watch_text_render_start_pos_y;
		...
		{
			s_watchTextYPos += g_pCVars->watch_text_render_size * g_pCVars->watch_text_render_lineSpacing;
			if (s_watchTextYPos + (g_pCVars->watch_text_render_size * g_pCVars->watch_text_render_lineSpacing) > gEnv->pRenderer->GetHeight())
			{
	...
	int CryWatchFunc(const char* message)
	{
		...
		if (gEnv && gEnv->pRenderer && CGamePlugin::GetInstance() && g_pCVars->watch_enabled)
		{
			float color[4] = { 1,1,1,1 };
			IFFont* pFont = gEnv->pCryFont->GetFont("default");
			float xscale = g_pCVars->watch_text_render_size * g_pCVars->watch_text_render_fxscale;
			...
			IRenderAuxText::Draw2dLabel(g_pCVars->watch_text_render_start_pos_x + s_watchTextXPos, yPos, g_pCVars->watch_text_render_size, color, false, "%s", message);
			return 1;
		}
