...
#define STATE_DEBUG
	...
	enum EStateEvent
	{
		...
		eStateEVent_Enter,
		eStateEVent_Exit,
		eStateEVent_Serialize,
		eStateEVent_PostSerialize,
		eStateEVent_NetSerialize,
		eStateEVent_Debug,

		eStateEVent_Custom = 100
	};
...
#define MAX_HSMEVENT_DATA	5
	...
	public:
		...
		SStateEvent() : m_eventType(eStateEVent_None)
#ifdef STATE_DEBUG
			, m_debugContextAt(-1)
#endif
		{
	...
	struct SStateEventSerialize : public SStateEvent
	{
		explicit SStateEventSerialize(TSerialize& ser)
			: SStateEvent(eStateEVent_Serialize)
		{
			AddData(&ser);
		}
...
#define CALL_STATE_DELETE_FN(subStateIndex)  (*(m_factories[trueStateID].m_deletePtr))
	...
	public:
		static void StateInit(HOST& host, CStateMachineRegistration<HOST>& statemachineReg, STATE*& pState)
		{
			...
			RecursiveToCommonReverse(host, pState->m_currentState, 0, pState, eStateEVent_Init);
			StateMachineHandleEventForState(host, statemachineReg, pState, STATE_DEBUG_RAW_EVENT_LOG(pState, eStateEVent_Enter), 0);
		}
		...
		static void StateRelease(HOST& host, CStateMachineRegistration<HOST>& statemachineReg, STATE*& pState)
		{
			...
			RecursiveToCommon(host, pState->m_currentState, 0, pState, STATE_DEBUG_RAW_EVENT_LOG(pState, eStateEVent_Exit));
			RecursiveToCommon(host, pState->m_currentState, 0, pState, eStateEVent_Release);
		}
		...
		static void StateTransition(HOST& host, CStateMachineRegistration<HOST>& stateMachineReg, STATE*& pActiveState)
		{
			...
			if (pendingEvent.GetEventId() != eStateEVent_None)
			{
			...
			do
			{
				...
				default:
					if (pState->m_currentState != stateResult)
					{
						...
						if (event.GetEventId() >= eStateEVent_Custom)
						{
	...
	protected:
		...
		void RequestTransitionState(HOST& host, TStateIndex stateTransition)
		{
			...
			RequestTransitionState(host, stateTransition, eStateEVent_None);
		}
	...
	private:
		...
		void TransitionFromCurrentToSubState(HOST& host, CStateMachineRegistration<HOST>& stateMachineReg, TStateIndex toSubState)
		{
			...
			CStateHelper<HOST, CStateHierarchy<HOST> >::RecursiveToCommon(host, m_currentState, commonParent, pState, eStateEVent_Exit);
			CStateHelper<HOST, CStateHierarchy<HOST> >::RecursiveToCommon(host, m_currentState, commonParent, pState, eStateEVent_Release);

			m_currentState = toSubState;

			CStateHelper<HOST, CStateHierarchy<HOST> >::RecursiveToCommonReverse(host, m_currentState, commonParent, pState, eStateEVent_Init);
			CStateHelper<HOST, CStateHierarchy<HOST> >::StateMachineHandleEventForState(host, stateMachineReg, pState, eStateEVent_Enter, commonParent);
		}
	...
	public:
		...
		void StateMachineUpdateTime(HOST& host, CStateMachineRegistration<HOST>& stateMachineReg, float frameTime, bool debug)
		{
			...
			if (debug)
			{
				...
				SStateEvent debugEvent(eStateEVent_Debug);
	...
	const SStateEvent SStateDebugContext::StateDebugAndLog(CStateHierarchy<HOST>* pState, const char* stateName, SStateEvent stateEvent)
	{
		...
		if (stateEvent.GetEventId() < eStateEVent_Custom)
		{
